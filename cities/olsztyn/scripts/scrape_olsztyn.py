#!/usr/bin/env python3
"""
Radoskop Olsztyn — BIP scraper (alpha).

Olsztyn nie używa eSesja (olsztynjurajski.esesja.pl to inny Olsztyn,
"Jurajski" w śląskim). Dane głosowań Rady Miasta Olsztyna (warmińsko-mazurskie)
są w BIP pod bip.olsztyn.eu jako PDF "Wykaz głosowań" per sesja.

Architektura analogiczna do scrape_czestochowa.py:

  1. Lista sesji z `https://bip.olsztyn.eu/bip/folder/3098/sesje_rady_miasta/`
     (URL pattern session detail: `https://bip.olsztyn.eu/{ID}/{slug}.html`).
  2. Strona detalu sesji ma sekcję "Załączniki" z PDF "Wykaz głosowań".
  3. PDF parser pdfplumber, format defensywny z fallbackiem.
  4. Output: data.json + profiles.json + kadencja-{id}.json.

Cache trzy poziomy: HTML pages, PDF files, parsed votes (sha256-based).

UWAGA (2026-05-14): scraper napisany bez ability to fetch BIP olsztyn z dev
maszyny (BIP zwraca pusto przez WAF/bot block). Selektory listy sesji
i wyciągania PDF mogą wymagać adjustacji po pierwszym runie. Plan:
NAS robi pierwszy run, wykleimy log, dolukamy parsery pod faktyczny markup.

Użycie:
  python3 scrape_olsztyn.py --output docs/data.json --profiles docs/profiles.json
...
BIP_BASE = "https://bip.olsztyn.eu"
SESSIONS_LIST_URL = f"{BIP_BASE}/kategoria/16/sesje-rady-miasta.html"
...
HEADERS = {
    "User-Agent": "Radoskop/1.0 (https://olsztyn.radoskop.pl; kontakt@radoskop.pl)",
    "Accept": "text/html,*/*",
}
DELAY = 0.3
TIMEOUT = 30
...
# ---------------------------------------------------------------------------
# Helpers: HTTP + cache (analogiczne do czestochowy)
# ---------------------------------------------------------------------------
...
def _surname_initial_key(name: str) -> str:
    """Klucz dla "first letter of first word + last word" (handle middle names).
    Np. 'Marcin Grzegorz Galibarczyk' → 'm galibarczyk'.
    """
...
def build_name_lookup() -> dict[str, str]:
    """Dual lookup: full normalized + (first_initial, last_name).
    ...
    for canonical_name in COUNCILORS:
        ...
        si = _surname_initial_key(canonical_name)
        if si and si not in out:
            out[si] = canonical_name
    return out
...
#   "XII Sesja Rady Miasta Olsztyna z dnia 26 marca 2025 roku"
#   "XXV Sesja Rady Miasta Olsztyna 29 kwietnia 2026 r."
#   "Uroczysta XVIII Sesja Rady Miasta 17 października 2025 r."
#   "II Nadzwyczajna Sesja Rady Miasta Olsztyna ..."
SESSION_TITLE_RE = re.compile(
    r"(?:Uroczysta\s+)?([IVXLCDM]+)\s+(Nadzwyczajna\s+)?[Ss]esja\s+Rady\s+Miast",
    re.IGNORECASE,
)
DATE_RE = re.compile(
...
# URL session detail: /{ID}/{slug-with-sesja}.html
SESSION_URL_RE = re.compile(r"^(?:https?://[^/]+)?/\d+/[^/]*sesja[^/]*\.html$", re.IGNORECASE)


def fetch_session_list(http_session: requests.Session, debug: bool = False, max_sessions: int = 0) -> list[SessionMeta]:
    """Pobiera paginated listę sesji z `/kategoria/16/sesje-rady-miasta.html`.
    Pagination: `?page=N` (page=1 to baseline). Linki sesji: `/{ID}/{slug-with-sesja}.html`.
    Tytuł podaje numer rzymski plus datę (czasem "z dnia DATA", czasem "DATA r.").
    """
    sessions: list[SessionMeta] = []
    seen_urls: set[str] = set()
    page = 1
    while True:
        url = SESSIONS_LIST_URL if page == 1 else f"{SESSIONS_LIST_URL}?page={page}"
        try:
        ...
        for a in soup.find_all("a", href=True):
            href = a["href"]
            if not SESSION_URL_RE.match(href):
                continue
            title = a.get_text(" ", strip=True)
            m = SESSION_TITLE_RE.search(title)
            if not m:
                continue
            number = m.group(1).upper()
            nadzwyczajna = bool(m.group(2))
        ...
        # Next page check: szukaj linka ?page={page+1}
...
# ---------------------------------------------------------------------------
# PDF parser (analogiczny do czestochowy)
# ---------------------------------------------------------------------------
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Radoskop Olsztyn, BIP scraper")
