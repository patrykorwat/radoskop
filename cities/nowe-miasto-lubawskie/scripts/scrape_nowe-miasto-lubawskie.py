...
# -*- coding: utf-8 -*-
"""
Radoskop Nowe Miasto Lubawskie — imienne głosowania Rady Miejskiej (AlfaTV "System Rada").

Źródło: https://rada.umnowemiasto.pl/ (AlfaTV System Rada, IX kadencja 2024-2029).
Pełne per-radny głosowania serwer-renderowane jako HTML (bez PDF/OCR).
Adapter wzorowany na cities/turek/scripts/scrape_turek.py (ta sama platforma).

Struktura:
  /glosowania                   -> lista sesji (link /glosowania/posiedzenie/{id} + data)
  /glosowania/posiedzenie/{id}  -> wszystkie głosowania sesji; accordion-item z tabelą
      "Imienny wykaz głosowania" (Imię i nazwisko | Głos: za/przeciw/wstrzymał się)
  /sklad-rady                   -> roster (/sklad-rady/radny/{id}); klub niedostępny
      na stronach radnych (brak dt/dd Przynależność) -> pusty (kluby do kuracji).

Użycie:
    python scrape_nowe_msto_lubawskie.py --city-dir cities/nowe-miasto-lubawskie [--cache-dir .cache]
"""
...
BASE = "https://rada.umnowemiasto.pl"
...
def discover_sessions(cache_dir):
    ...
    for a in soup.find_all("a", href=re.compile(r"/glosowania/posiedzenie/\d+")):
        ...
        # "Zawiadomienie o X sesji" = ogłoszenie, 0 głosowań — nie sesja
        if re.search(r"Zawiadomienie", txt, re.I):
            continue
        dm = re.search(r"(\d{4}-\d{2}-\d{2})", txt)
        mid = int(re.search(r"(\d+)$", a["href"]).group(1))
        nm = re.search(r"([IVXLCDM]+)\s+[Ss]esj", txt)
        vc = re.search(r"\s(\d+)\s+Sprawd", txt)
        out.append({"id": mid, "name": (nm.group(1) + " sesja") if nm else "",
                    "date": dm.group(1) if dm else "",
                    "vote_count": int(vc.group(1)) if vc else None})
    seen = set(); uniq = []
    for s in out:
        if s["id"] in seen:
            continue
...
def parse_session_votes(html):
    ...
    for item in soup.find_all("div", class_="accordion-item"):
        ...
        big = [tb for tb in tables if len(tb.find_all("tr")) > 10]
        if not big:
            continue
        tbl = big[-1]
        rows = []
...
def build_output(records, roster):
    ...
    for name in all_names:
        councilors_data[name] = {"name": name, "club": _club_key(name, roster),
    ...
    for a, b in combinations(names_sorted, 2):
        ...
        pairs.append({"a": a, "b": b, "club_a": _club_key(a, roster),
                      "club_b": _club_key(b, roster),
                      "score": round(same / len(common) * 100, 1), "common_votes": len(common)})
    pairs.sort(key=lambda x: x["score"], reverse=True)

    club_counts = Counter(_club_key(n, roster) for n in all_names)
    kad = {
...
def build_profiles(records, roster):
    ...
    n_rec = len([1 for r in records if r["session_date"] >= KAD_START])
    for nm in sorted(cv.keys()):
        ...
        profiles.append({
            ...
            "kadencje": {KADENCJA_ID: {
                "club": _club_key(nm, roster), "has_voting_data": True,
...
def main():
    ...
    print(f"[nml] total votes={output['kadencje'][0]['total_votes']} "
