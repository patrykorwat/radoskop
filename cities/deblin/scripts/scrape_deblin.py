#!/usr/bin/env python3
"""
Radoskop Dęblin — eSesja scraper (thin wrapper around scripts/lib_esesja.py).

Skład rady + przypisania klubowe są wczytywane z config.json (sekcja
club_assignments). Format: {"Imię Nazwisko": "kod_klubu"}.

Backend: https://deblin.esesja.pl (Rada Miasta Dęblin, IX kadencja 2024-2029).
...
if __name__ == "__main__":
    raise SystemExit(EsesjaScraper(
        base_url="https://deblin.esesja.pl",
        kadencje=KADENCJE,
        councilors=COUNCILORS,
    ).run_cli(prog_name="Radoskop Dęblin (https://deblin.esesja.pl)"))
