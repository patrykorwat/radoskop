#!/usr/bin/env python3
"""Scraper interpelacji i zapytań radnych Rady Miasta Chorzowa (IX kad. 2024-2029).

Źródło: eSesja (https://chorzow.esesja.pl) — moduł "Interpelacje i zapytania".

    https://chorzow.esesja.pl/interpelacje_i_zapytania/{page}
...
Struktura listingu (paginowany — patrz _last_page / pager):
  ...
  W tym źródle TYTUŁ listingu = przedmiot (temat interpelacji) wprost. Odpowiedzi (jeśli są) jako PDF do pobrania z detalu.
  ...
  Chorzów: odpowiedzi publikowane jako PDF w detailu (jeśli udzielono) osobnych odpowiedzi (brak bloków
...
def main() -> int:
    global _DEBUG
    parser = argparse.ArgumentParser(description="Scraper interpelacji — Chorzów (eSesja)")
    ...
    print("=== Interpelacje — Chorzów (eSesja) ===")
    ...
    for it in items:
        ...
        cri = f"cri-chorzow-{idm.group(1)}" if idm else f"cri-chorzow-{len(records)}"
