...
# -*- coding: utf-8 -*-
"""
Radoskop Złocieniec — imienne głosowania Rady Miejskiej w Złocieniecie (AlfaTV "System Rada").
...
Użycie:
    python scrape_rypin.py --city-dir cities/rypin [--cache-dir .cache]
"""
...
def _club_key(name, roster):
    ...
    if not club:
        return "NZ"
    cl = club.lower()
    if "ziemia turkowska" in cl:
        return "ZT"
    if "lewic" in cl:
        return "LEW"
    if "razem dla turku" in cl or "razem" in cl:
        return "RDT"
    if "laczy nas rypin" in cl or "łączy nas rypin" in cl:
        return "LNT"
    # electoral committees (KWW/KOALICYJNY KWW) = brak klubu -> niezrzeszeni
...
def main():
    ...
    print(f"[rypin] total votes={output['kadencje'][0]['total_votes']} "
