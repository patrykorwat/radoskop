#!/usr/bin/env python3
"""Scraper interpelacji i zapytań radnych Rady Miejskiej w Zambrowie (IX kad. 2024-2029).

Źródło: eSesja (https://zambrow.esesja.pl) — moduł "Interpelacje i zapytania".

    https://zambrow.esesja.pl/interpelacje_i_zapytania/{page}

Struktura listingu (paginowany rejestr, ta sama platforma co Sieradz/Człuchów):
  Każdy rekord = <div class="user-item"> w głównym rejestrze:
      <p class="title"><a href="/interpelacja/{id}_{hash}/{slug}.htm">...</a></p>
      <p class="subtitle"><autorzy / przedmiot>Radny</autorzy> - {Wniosek|Interpelacja|Zapytanie} z dnia {DD miesiąc YYYY}</p>
  Typ (interpelacja/zapytanie/wniosek) i data z subtitle; radny z subtitle.
  Autorzy zbiorowi (przecinek / "i" / "oraz") -> radny="" (nie zgadujemy).

  Detal (wpis/iinfo jak u Sieradza):
      <div class='wpis'><p>{przedmiot}</p></div><div class='iinfo'>{data} - <b>{Radny}</b></div>
      <div class='wpis'><p>Załącznik do {Typ} ({file})</p></div><div class='iinfo'><a ... href='...pdf'>Pobierz plik</a></div>
      <div class='wpis'><p>Odpowiedź do {Typ} ({file})</p></div>  (jeśli udzielono)
...
# subtitle: "Stanisław Klimek - Wniosek z dnia 30 lipca 2026"
_SUB_RE = re.compile(
    r"^(?P<radny>.+?)\s*-\s*(?P<typ>interpelacj[ae]|zapytani[ae]|wniosek|zapytani[ae]|wniosk[ie])\s+z dnia\s+"
...
def main() -> int:
    global _DEBUG
    parser = argparse.ArgumentParser(description="Scraper interpelacji — Tarnów (eSesja)")
    ...
    print("=== Interpelacje — Zambrów (eSesja) ===")
    ...
    for it in items:
        ...
        cri = f"cri-zambrow-{idm.group(1)}" if idm else f"cri-zambrow-{len(records)}"
    ...
    wnioski = sum(1 for r in records if r["typ"] == "wniosek")
    answered = sum(1 for r in records if r["odpowiedz_status"] == "Udzielono")
    no_radny = sum(1 for r in records if not r["radny"])
    print("\n=== Podsumowanie ===")
    print(f"Interpelacje: {interp} | Zapytania: {zap} | Wnioski: {wnioski} | Z odpowiedzią: {answered} | Bez radnego: {no_radny} | Razem: {len(records)}")
