#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Radoskop ziebice — Tier-2 (roster / "model berliński") scraper.

Brak imiennych glosowań IX kadencji. Miasto dodawane jako Tier-2:
skład rady (roster). Rada Miejska w Ziębicach.

Źródło rosteru: ziebice.biuletyn.net/?bip=1&cid=1432 (Radni w kadencji 2024-2029)
"""
import json
import re
import sys
...
KAD = "2024-2029"
NAMES = ["Jolanta Bednarz", "Mariusz Jaźwiec", "Bartłomiej Kąfera", "Kamila Kocjan", "Marek Kuriata", "Artur Piękarski", "Jan Przyszlakiewicz", "Beata Pustułka", "Mateusz Rynka", "Wiesław Sala", "Janusz Sobol", "Edyta Sopuch", "Tomasz Zapotoczny"]
...
def build(city_dir: Path) -> int:
    ...
    names = list(NAMES)
    print(f"  ziebice roster: {len(names)}")
    kadencja = {
