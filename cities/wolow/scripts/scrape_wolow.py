#!/usr/bin/env python3
"""Radoskop Wołów — eSesja scraper (thin wrapper around scripts/lib_esesja.py).

Skład rady + przypisania klubowe są wczytywane z config.json (sekcja
club_assignments). Format: {"Imię Nazwisko": "kod_klubu"}.

Backend: https://wolow.esesja.pl (Rada Miejska w Wołowie, IX kadencja 2024-2029).
...
if __name__ == "__main__":
    raise SystemExit(EsesjaScraper(
        base_url="https://wolow.esesja.pl",
        kadencje=KADENCJE,
        councilors=COUNCILORS,
    ).run_cli(prog_name="Radoskop Wołów (https://wolow.esesja.pl)"))
