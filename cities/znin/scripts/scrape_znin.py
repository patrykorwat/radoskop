...
# -*- coding: utf-8 -*-
"""
Radoskop Żnin — imienne głosowania Rady Miejskiej w Żninie (IX kadencja 2024-2029).

Źródło: AlfaTV "System Rada" dla Urzędu Miejskiego w Żninie
(rada.gminaznin.pl, /glosowania → /glosowania/posiedzenie/{id}).
...
Użycie: python scrape_znin.py --output docs/data.json --profiles docs/profiles.json
        [--cache-dir .cache]
"""
...
def main():
    ...
    k=output['kadencje'][0]
    print(f"[znin] sesje: {k['total_sessions']}, glosowania: {k['total_votes']}, radni: {k['total_councilors']}")
    print(f"[znin] walidacja agregatow: ok={validated['ok']} mismatch={validated['mismatch']} noagg={validated['noagg']} emptytable={validated['emptytable']}")
