...
# -*- coding: utf-8 -*-
"""
Radoskop Złocieniec — imienne głosowania Rady Miejskiej w Rypinie (AlfaTV "System Rada").

Źródło: https://rada.zlocieniec.pl/ (AlfaTV System Rada, IX kadencja 2024-2029).
...
Struktura:
  ...
  /glosowania/posiedzenie/{id} -> wszystkie głosowania 1 sesji w HTML; każdy blok
      ...
      agregatów (za/wstrzymujące/przeciw/nieoddane/nieobecni) oraz tabelą
  ...
  /sklad-rady/radny/{id}      -> nazwisko + Klub (Przynależność...Klub)

27 sesji IX kadencji z glosowaniami (I 2024-05-06 .. XXX 2026-07-13), 15 radnych.

Użycie:
    python scrape_rypin.py --city-dir cities/zlocieniec [--cache-dir .cache]
"""
...
def _club_key(name, roster):
    ...
    if not club:
        return "NZ"
    cl = club.lower()
    # All Złocieniec affiliations are KWW electoral committees (KWW - MOŻNA LEPIEJ 2024,
    # KWW AKTYWNI DLA GMINY, KWW LOKALNE POROZUMIENIE SAMORZĄDOWE) — electoral committees
    # are not city-council clubs -> niezrzeszeni (NZ).
    if "komitet wyborczy" in cl or "kww" in cl or "koalicyjny" in cl:
        return "NZ"
    return "NZ"
...
def main():
    ...
    print(f"[zlocieniec] total votes={output['kadencje'][0]['total_votes']} "
