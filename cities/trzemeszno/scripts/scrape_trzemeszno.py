#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Radoskop trzemeszno — Tier-2 (roster / "model berliński") scraper.

Brak imiennych glosowań IX kadencji. Miasto dodawane jako Tier-2:
skład rady (roster). Rada Miejska w Trzemesznie.

Źródło rosteru: bip.trzemeszno.pl/rada-miejska-trzemeszna-kadencja-2024-2029.html
"""
import json
import re
import sys
...
KAD = "2024-2029"
NAMES = ["Agnieszka Bartz", "Tadeusz Pawlak", "Daniel Bisikiewicz", "Marzena Brzewińska", "Maciej Dalewski", "Marcin Jakubiak", "Bożena Kupczyk", "Sebastian Kwieciński", "Andrzej Lisiecki", "Aleksandra Majchrzak", "Benedykt Nitka", "Agnieszka Oleksy", "Radosław Piechowiak", "Hieronim Stopczyński", "Dominik Woźniak"]
...
def build(city_dir: Path) -> int:
    ...
    names = list(NAMES)
    print(f"  trzemeszno roster: {len(names)}")
    kadencja = {
