#!/usr/bin/env python3
"""Radoskop Sokołów Podlaski — eSesja scraper (thin wrapper around scripts/lib_esesja.py).

Skład rady + przypisania klubowe są wczytywane z config.json (sekcja
club_assignments). Format: {"Imię Nazwisko": "kod_klubu"}.

Backend: https://sokolowpodlaski.esesja.pl (Rada Miejska w Sokołowie Podlaskim,
...
if __name__ == "__main__":
    raise SystemExit(EsesjaScraper(
        base_url="https://sokolowpodlaski.esesja.pl",
        kadencje=KADENCJE,
        councilors=COUNCILORS,
    ).run_cli(prog_name="Radoskop Sokołów Podlaski (https://sokolowpodlaski.esesja.pl)"))
