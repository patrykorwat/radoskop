#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Radoskop szamotuly — Tier-2 (roster / "model berliński") scraper.

Brak imiennych glosowań IX kadencji. Miasto dodawane jako Tier-2:
skład rady (roster). Rada Miasta i Gminy w Szamotułach.

Źródło rosteru: szamotuly.pl/rada-miasta-i-gminy.html (Skład Rady IX kad.)
"""
import json
import re
import sys
...
KAD = "2024-2029"
NAMES = ["Justyna Szaniawska-Budaj", "Bartosz Węglewski", "Anna Gaj", "Katarzyna Mejer", "Marta Więcławska", "Maciej Magdziarek", "Marian Płachecki", "Marek Kasprzyk", "Joanna Gzyl", "Natalia Wojciechowska", "Mikołaj Trąbczyński", "Joanna Ludwiczak", "Krzysztof Dyzert", "Marian Juś", "Dominika Buchwald", "Dawid Pius", "Tomasz Łączkowski", "Jerzy Najderek", "Bernadeta Markiewicz", "Ilona Kaluga", "Łukasz Heckert"]
...
def build(city_dir: Path) -> int:
    ...
    names = list(NAMES)
    print(f"  szamotuly roster: {len(names)}")
    kadencja = {
