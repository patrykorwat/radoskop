#!/usr/bin/env bash
# Radoskop Zduńska Wola — eSesja PM-A: scrape glosowania imienne + generate_site.
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CITY_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"

echo "[zdunska-wola] scrape_zdunska_wola.py"
python3 "$SCRIPT_DIR/scrape_zdunska_wola.py" \
  --output "$CITY_DIR/docs/data.json" \
  --profiles "$CITY_DIR/docs/profiles.json" \
  --cache-dir "${RADOSKOP_CACHE_DIR:-/cache/zdunska-wola}/html"
