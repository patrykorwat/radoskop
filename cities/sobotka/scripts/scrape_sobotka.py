...
# -*- coding: utf-8 -*-
"""
Radoskop Sobótka — imienne głosowania Rady Miejskiej w Miastku (AlfaTV "System Rada").

Źródło: https://rada.sobotka.pl/ (AlfaTV System Rada, IX kadencja 2024-2029).
...
Struktura:
  ...
  /sklad-rady/radny/{id}      -> nazwisko (kluby NIE publikowane u Sobótki)

34 sesje IX kadencji (2024-05 .. 2026-08), 16 radnych.
Adapter rada.sobotka.pl == rada.miastoturek.pl (ten sam platformowy szablon AlfaTV).

Użycie:
    python scrape_sobotka.py --city-dir cities/sobotka [--cache-dir .cache]
"""
...
def parse_session_votes(html):
    ...
    for item in soup.find_all("div", class_="accordion-item"):
        ...
        # Sobótka: identyfikujemy tabelę imienną po nagłówku 'Imię i nazwisko | Głos'
        # (nie po liczbie wierszy — radni bywają nieobecni i tabela ma <16 wierszy)
        big = [tb for tb in tables if "Imi" in (tb.get_text(" ", strip=True)[:200])
               and "G\u0142os" in tb.get_text(" ", strip=True)[:200]]
        if not big:
            continue
        tbl = big[0]
        rows = []
...
def _club_key(name, roster):
    # Sobótka: System Rada nie publikuje klubów (Przynależność brak na stronach
...
def main():
    ...
    print(f"[sobotka] total votes={output['kadencje'][0]['total_votes']} "
